<!-- src/lib/components/Card_NoGuessWork.svelte -->
<script>
...
<div 
  ...
  style="{background ? `background-image: url('${background}'); background-position: center; background-size: cover;` : 'background: linear-gradient(to bottom right, #c3cde1, #dde3ee)'}">
    <!-- Mock Browser Window -->
    <div class="flex h-full flex-col overflow-hidden rounded-xl bg-white/10 shadow-inner">
      ...
      <div class="flex flex-grow flex-col justify-center items-center pt-0">
        ...
        <div class="w-full" style="aspect-ratio: 1.22/1; max-height: 280px;">
          ...
          <div slot="before">
            <img 
              src="/BeforeB.png" 
              alt="Job post before improvements"
              class="image-slider-content"
            />
          </div>
  
          <!-- The "After" state (with AI recommendations) -->
          <div slot="after">
            <img 
              src="/AfterB.png" 
              alt="Job post after AI-driven improvements"
              class="image-slider-content"
            />
          </div>
          </ImageSlider>
      ...
      <!-- <div class=" h-full">
        <div class="p-5" style="width: 100%; height: 100%;">
          <div class="h-full w-full rounded-3xl" style="background: linear-gradient(to right, #ef4444, #f59e0b, #10b981); opacity: 0.4;"></div>
        </div>
      </div>   -->
     </div> 
    </div>
