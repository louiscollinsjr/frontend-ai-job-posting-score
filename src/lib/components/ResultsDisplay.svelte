  ...
  async function downloadReport() {
    ...
    if (typeof html2pdf === 'undefined') {
      ...
      try {
        ...
        alert('Could not load the PDF generator. Please try again later.');
        return;
      }
    }
    
    // Clone the results content to create a downloadable version
    const resultsContent = document.querySelector('.results-page')?.cloneNode(true);
    
    if (!resultsContent) {
      alert('Could not find results content to download');
      return;
    }
    
    // Remove action buttons from the cloned content
    const actionButtons = resultsContent.querySelectorAll('.flex.flex-wrap.justify-center.gap-4.mt-8.bg-gray-100');
    actionButtons.forEach(element => element.remove());
    ...
    header.innerHTML = `
      <h1 style="font-size: 24px; margin-bottom: 5px;">JobPostScore Report</h1>
    ...
    footer.innerHTML = `<p>Generated by JobPostScore - ${new Date().toLocaleDateString()}</p>`;
    pdfContainer.appendChild(footer);
    
    // Temporarily add the container to the document (invisible)
    pdfContainer.style.position = 'absolute';
    pdfContainer.style.left = '-9999px';
    document.body.appendChild(pdfContainer);
    
    // Generate a filename
    const jobTitle = results?.job_title || 'job-posting';
    const fileName = `job-score-report-${jobTitle.toLowerCase().replace(/\s+/g, '-')}.pdf`;
    ...
    try {
      ...
      // Find the specific Download Report button
      const downloadButton = Array.from(document.querySelectorAll('button'))
        .find(btn => btn.innerText.trim() === 'Download Report');
      const originalButtonText = downloadButton ? downloadButton.innerText : 'Download Report';
      if (downloadButton) downloadButton.innerText = 'Generating PDF...';
      
      // Generate the PDF
      await html2pdf().set(opt).from(pdfContainer).save();
      
      // Reset the button text
      if (downloadButton) downloadButton.innerText = originalButtonText;
    } catch (error) {
